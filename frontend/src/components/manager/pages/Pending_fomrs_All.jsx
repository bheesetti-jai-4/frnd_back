...
import { pending_Forms_manager, searchformsbymanager } from "../../../server/Manager_Axios";
import { toast } from "react-toastify";

function Pending_fomrs_All() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await pending_Forms_manager(); // Fetch all form history
  ...
  const handleSearch = async (e) => {
    ...
    try {
      const response = await searchformsbymanager(query);
  ...
  return (
    <>
      <section className="bg-gray-100 py-10">
        <div className="container mx-auto px-4">
          <div className="max-w-2xl mx-auto text-center">
            <h2 className="text-3xl font-bold mb-6  ">Pending Forms History</h2>
...
export default Pending_fomrs_All;
